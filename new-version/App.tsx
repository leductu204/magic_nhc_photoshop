...
import { generateSpeech, translateText, generateStyledImage, generateProfileImage, preprocessClothImage, analyzeImageText, generateClothingSwap, fileToPart, refineClothingResult, generatePaintingImage, analyzeRestorationImage, generateBackgroundSwapImage, generateFaceStraightenImage, generateLifestyleImage, generateMockupImage, generateLightingEffectImage, generateRemoveBackgroundImage, generateSymmetricEditImage, generateBabyUltrasoundImage, generateArchitectureRenderImage, generateBabyFromUltrasoundImage, generateUpscaleExpandImage } from './services/geminiService';
...
const DEFAULT_GENERATION_SETTINGS: GenerationSettings = {
    ...
    modelType: 'flash' as ModelType,
    imageSize: '2K' as '1K' | '2K' | '4K' | '5K',
    aspectRatio: 'auto',
      ...
      userPrompt: `Phục chế tấm ảnh cũ này với chất lượng cao: xóa vết trầy xước, bụi, vết bẩn, nếp gấp, phục hồi các vùng bị mờ, chỉnh sáng và độ tương phản, làm nét các chi tiết trên khuôn mặt, tăng cường màu sắc tự nhiên, giữ nguyên kết cấu gốc, giữ tông da chân thực, không làm da quá mịn giả.
{
  ...
  const generateSingleImage = async (file: File, id: string, isPortrait: boolean) => {
      ...
      try {
        let url = '';
        if (viewMode === 'profile') url = await generateProfileImage(file, profileSettings);
  ...
  const handleRefineRestoration = async () => {
        ...
        try {
            ...
            if (viewMode === 'restoration') {
                ...
                const url = await generateProfileImage(file, profileSettings);
                     ...
                     <button 
                      onClick={() => {
                        ...
                        images.forEach((img, i) => {
                          ...
                          }, i * 200); // Small delay to prevent browser blocking multiple downloads
                        });
                      }} 
