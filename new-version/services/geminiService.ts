...
const UPSCALER_5K_PROMPT_OBJECT = { "resolution_target": "5K" };

const getAI = (modelType?: string, customApiKey?: string) => {
    const apiKey = customApiKey || ((modelType === 'pro-image' || modelType === 'nano-pro') ? process.env.API_KEY : process.env.GEMINI_API_KEY);
...
export const generateBabyUltrasoundImage = async (
    ...
    try {
        ...
        const selectedModel = (modelType === 'pro-image') ? 'gemini-3.1-pro-preview' : (modelType === 'nano-pro') ? 'gemini-3.1-flash-image-preview' : 'gemini-2.5-flash-image';
        ...
        const response = await ai.models.generateContent({
            ...
            config: (modelType === 'pro-image' || modelType === 'nano-pro') ? { imageConfig: { imageSize: '4K', aspectRatio: 'auto' } } : undefined
        });
...
export const generateStyledImage = async (
  ...
  try {
    ...
    if (modelType === 'pro-image' || modelType === 'nano-pro') {
        ...
        if (imageSize === '5K') {
            ...
            extra5KPrompt = `\n\n[SYSTEM MODE: 5K ULTRA UPSCALE]\nEXECUTE "upscale_5k.py" SIMULATION LOGIC:\n${JSON.stringify(UPSCALER_5K_PROMPT_OBJECT, null, 2)}`;
        }
    }
    ...
    const response = await ai.models.generateContent({
      ...
      config: (modelType === 'pro-image' || modelType === 'nano-pro') ? { imageConfig: { imageSize: effectiveImageSize, aspectRatio: targetRatio } } : undefined
    });
...
export const generateBackgroundSwapImage = async (
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            config: (modelType === 'pro-image' || modelType === 'nano-pro') ? { imageConfig: { imageSize: imageSize === '5K' ? '4K' : imageSize, aspectRatio: aspectRatio === 'auto' ? '1:1' : aspectRatio } } : undefined
        });
...
export const generatePaintingImage = async (originalFile: File, settings: GenerationSettings): Promise<string> => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: selectedModel,
            contents: { parts: contentParts },
            config: modelType === 'pro-image' ? { imageConfig: { imageSize: imageSize === '5K' ? '4K' : imageSize, aspectRatio: aspectRatio === 'auto' ? '1:1' : aspectRatio } } : undefined
        });
