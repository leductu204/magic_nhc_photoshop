              ...
              <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                      ...
                      <div className="text-[18px] font-black text-white uppercase leading-none tracking-tight">
                        {settings.modelType === 'pro-image' ? 'BANANA 3.1 PRO' : settings.modelType === 'nano-pro' ? 'NANO BANANA 3.1 PRO' : 'BANANA MIỄN PHÍ'}
                      </div>
                  </div>
              </div>
          ...
          {isModelSelectorOpen && (
              <div className="px-6 pb-6 pt-2 space-y-6 border-t border-white/5 animate-in fade-in slide-in-from-top-2 duration-300">
                  ...
                  <div className="grid grid-cols-3 gap-2 p-1 bg-black/40 rounded-xl border border-white/5">
                      <button 
                        onClick={() => onSettingsChange({ modelType: 'flash' })}
                        className={`py-2.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${settings.modelType === 'flash' ? 'bg-zinc-800 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
                      >
                        Miễn Phí
                      </button>
                      <button 
                  ...
                  {(settings.modelType === 'pro-image' || settings.modelType === 'nano-pro') ? (
                      <>
                        <div className="space-y-4">
                            <h3 className={`text-[16px] font-black ${settings.modelType === 'nano-pro' ? 'text-violet-400' : 'text-sky-400'} uppercase tracking-widest`}>KÍCH THƯỚC</h3>
                            <div className="grid grid-cols-4 gap-3">
                                {['1K', '2K', '4K', '5K'].map((size) => (
                                    <button
                                        key={size}
                                        ...
                                        className={`py-4 rounded-xl border-2 font-black text-[16px] transition-all flex items-center justify-center ${
                                            ...
                                            ? `${settings.modelType === 'nano-pro' ? 'bg-violet-500 border-violet-400' : 'bg-sky-500 border-sky-400'} text-white shadow-[0_0_20px_rgba(139,92,246,0.4)]` 
                                            : 'bg-zinc-900/50 border-white/5 text-gray-500 hover:border-white/20'
                                        } ${size === '5K' && settings.imageSize !== '5K' ? 'text-red-500/60' : size === '5K' && settings.imageSize === '5K' ? 'bg-red-600 border-red-500' : ''}`}
                                    >
                                        {size}
                                    </button>
                                ))}
                        ...
                        <div className="space-y-4 pt-4 border-t border-white/5">
                            <p className="text-[10px] text-gray-400 italic leading-relaxed">
                                * Bạn cần nhập hoặc chọn API Key trả phí để sử dụng mô hình {settings.modelType === 'nano-pro' ? 'Nano Pro' : 'Bản Pro'}. Xem <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noreferrer" className={`${settings.modelType === 'nano-pro' ? 'text-violet-400' : 'text-sky-400'} underline hover:opacity-80`}>tài liệu thanh toán</a>.
                            </p>
                            ...
                            <div className="space-y-3">
                                ...
                                <button
                                    ...
                                    className={`w-full py-4 ${settings.modelType === 'nano-pro' ? 'bg-violet-500/5 border-violet-500/30 text-violet-400 hover:bg-violet-500/10' : 'bg-sky-500/5 border-sky-500/30 text-sky-400 hover:bg-sky-500/10'} border rounded-xl flex items-center justify-center gap-3 group transition-all active:scale-[0.98]`}
                                >
